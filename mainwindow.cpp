...
MainWindow::MainWindow(QWidget *parent) :
    QMainWindow(parent),
        ...
        writeModel(new WriteRegisterModel(this))  // 初始化 writeModel
{
    ui->setupUi(this);
...
MainWindow::~MainWindow()
{
    delete ui;
        ...
        // delete serial; // 不需要手动删除，Qt 会自动管理
        if (modbusDevice) {
...
void MainWindow::setupUiComponents() {
    ...
    // 应用样式表到按钮
    QList<QPushButton*> buttons = {ui->openSetupButton, ui->BASIC_btn, ui->IMU_btn, ui->WAVE_btn, ui->modbusButton, ui->modmstButton};
    ...
    ui->textEditWriteResult->setColumnHidden(2,1);
...
void MainWindow::setupPlot()
{
    // Configure the plotWidget for three generic data streams
       QStringList curveNames = {"Roll", "Pitch", "Yaw"};
       QVector<QColor> curveColors = {Qt::red, Qt::green, Qt::blue};

       for (int i = 0; i < 3; ++i) {
           ui->plotWidget->addGraph();
           ui->plotWidget->graph(i)->setPen(curveColors[i]);
           ui->plotWidget->graph(i)->setProperty("originalColor", curveColors[i]);
           ui->plotWidget->graph(i)->setName(curveNames[i]);
       }
...
void MainWindow::on_openSetupButton_clicked() {
    if (!setupDialog) {
        ...
        connect(setupDialog, &QObject::destroyed, this, [this]() { setupDialog = nullptr; });
...
void MainWindow::configureSerialPort(const QString &portName, int baudRate,
...
void MainWindow::processICM20602Data(const QByteArray &data) {
    ...
    statusBar()->showMessage(QString("ICM20602 - ax: %1, ay: %2, az: %3, mx: %4, my: %5, mz: %6")
                                 .arg(currentAx).arg(currentAy).arg(currentAz)
                                 .arg(currentMx).arg(currentMy).arg(currentMz));
...
void MainWindow::sendSerialData() {
    QString hexData = ui->textInput->text();
...
void MainWindow::updateDisplay(bool showOpenGL, bool sendMessage, bool showPlot, bool showModbus,bool showModSlv) {
    if (sendMessage) {
            ui->stackedWidget->setCurrentWidget(ui->BASICWidget);
        } else if (showModbus) {
            ui->stackedWidget->setCurrentWidget(ui->modbusGroup);
        } else if (showPlot) {
            ui->stackedWidget->setCurrentWidget(ui->WAVEWidget);
            ui->plotWidget->setVisible(showPlot);
        } else if (showOpenGL) {
            ui->stackedWidget->setCurrentWidget(ui->IMUWidget);
        } else if (showModSlv) {
            ui->stackedWidget->setCurrentWidget(ui->modslvGroup);
        }
}
...
void MainWindow::ensureSerialConnection() {
    if (!serial->isOpen()) {
        ...
        // 重新打开并配置串口
        configureSerialPort(serial->portName(), serial->baudRate(), serial->dataBits(),
                            serial->stopBits(), serial->parity(), serial->flowControl());
...
void MainWindow::configureModbusConnection() {
    ...
    if (connectionType == "TCP") {
        if (ui->lineEditIpAddress->text().isEmpty())
                    ui->lineEditIpAddress->setText(QLatin1String("127.0.0.1:502"));
        if (ui->lineEditAddress->text().isEmpty())
                    ui->lineEditAddress->setText(QLatin1String("1"));
    ...
    if(modbusDevice){
        connect(modbusDevice, &QModbusClient::stateChanged,
                this, &MainWindow::onStateChanged1);
    }
}
...
void MainWindow::on_readButton_clicked()
{
    if (!modbusDevice || modbusDevice->state() != QModbusDevice::ConnectedState) {
        ...
        QString portText = ui->lineEditAddress->text();
        int serverAddress = portText.isEmpty() ? 1 : portText.toInt();

        if (auto *reply = modbusDevice->sendReadRequest(QModbusDataUnit(table, startAddress, numEntries), serverAddress)) {
            if (!reply->isFinished()) {
...
void MainWindow::on_writeButton_clicked() {
    ...
    const auto table =
            static_cast<QModbusDataUnit::RegisterType>(ui->writeTable->currentData().toInt());
    // Get server address, default to 1 if empty
    QString addressText = ui->lineEditAddress->text();
    int serverAddress = addressText.isEmpty() ? 1 : addressText.toInt();
    if (sendNum < 1) {
            sendNum = 1; // 确保至少发送一个寄存器
        }


    QModbusDataUnit writeUnit(table, startAddress, sendNum);
    ...
    if (reply->error() == QModbusDevice::NoError) {
        ...
        for (uint i = 0; i < unit.valueCount(); i++) {
            const QString entry = tr("Address: %1, Value: %2").arg(unit.startAddress() + i)
                        ...
                        ui->textEditReadResult->addItem(entry);
        }
...
void MainWindow::onStateChanged1(int state)
{
    //bool connected = (state != QModbusDevice::UnconnectedState);
    //ui->actionConnect->setEnabled(!connected);
    //ui->actionDisconnect->setEnabled(connected);
...
void MainWindow::onWriteTableChanged(int index)
{
    ...
    if (coilsOrHolding) {
        ui->textEditWriteResult->setColumnHidden(1, index != 0);
        ui->textEditWriteResult->setColumnHidden(2, index != 3);
        ui->textEditWriteResult->resizeColumnToContents(0);
    }

    ui->readwriteButton->setEnabled(index == 3);
    ui->writeButton->setEnabled(coilsOrHolding);
    ui->labelStartAddressRead->setEnabled(coilsOrHolding);
    ui->spinBoxStartAddressRead->setEnabled(coilsOrHolding);
    ui->labelNumValuesRead->setEnabled(coilsOrHolding);
    ui->spinBoxNumValuesRead->setEnabled(coilsOrHolding);
    //ui->readButton->setEnabled(coilsOrHolding);
